...
import { CONTENT_ASPECT_RATIOS } from "@/lib/content-editor/media-input";
...
const KIND_LABELS: Record<ContentMediaInput["kind"], string> = {
...
export type ContentMediaItem = ContentMediaInput & { uid: string };

type MediaEditorProps = {
  items: ContentMediaItem[];
  onChange: (items: ContentMediaItem[]) => void;
...
export function MediaEditor({
  ...
  const update = (index: number, patch: Partial<ContentMediaItem>) => {
  ...
  const add = (kind: ContentMediaInput["kind"]) => {
    onChange([
      ...items,
      {
        uid: localUid(),
    ...
    <div>
      ...
      {items.length === 0 && (
        <p className="rounded-control border border-dashed border-border px-3 py-2.5 text-xs text-muted">
          {emptyHint ??
            "No media yet. YouTube videos, images and GIFs are supported via URL."}
        </p>
      )}
            ...
            >
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <SelectField
                  ...
                  value={item.kind}
                  onChange={(event) =>
                    update(index, { kind: event.target.value as ContentMediaItem["kind"] })
                  }
                  options={(Object.keys(KIND_LABELS) as ContentMediaItem["kind"][]).map(
                ...
                <SelectField
                  ...
                  options={CONTENT_ASPECT_RATIOS.map((ratio) => ({
                    value: ratio,
                    label: ratio,
                  }))}
                />
